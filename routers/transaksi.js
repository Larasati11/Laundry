    ...
    .then(result =>{
        ...
        .then(result => {
            return response.json({
                message: "Data transaksi behasil ditambahkan"
            })
        })
        .catch(error => {
            return response.json({
...
app.put("/:id_transaksi",async (request,response)=> {
    ...
    .then(async (result) => {
        ...
        .then(result => {
            return response.json({
                message: "Data transaksi behasil diupdate"
            })
        })
        .catch(error => {
            return response.json({
    ...
    //setelah berhasil insert ke tabel transaksi,
    //data dtail transaksi yang lama dihapus semua berdasarkan id_transaksinya
...
//endpoint untuk merubah status transaksi
...
//endpoint merubah status pembayaran
app.get("/bayar/:id_transaksi", (request, response) => {
    ...
    let data = {
        //mendapatkan tanggal yg saa ini berjalann
        tgl_bayar: new Date().toDateString().split("T")[0],
        dibayar: true
    }
